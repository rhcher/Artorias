...
local function _73_()
  ...
  local function _80_(bufnr, filetype, buftype)
    return ftmap[filetype]
  end
  return ufo.setup({provider_selector = _80_})
end
local function _81_()
...
local function _83_()
  ...
  return surround.setup({surrounds = {["("] = {add = {"(", ")"}, find = _84_, delete = "^(.)().-(.)()$"}, [")"] = {add = {"( ", " )"}, find = _85_, delete = "^(. ?)().-( ?.)()$"}}, move_cursor = false})
end
local function _86_()
...
local function _92_()
  ...
  do
    ...
    vim.keymap.set("n", "<leader>bk", _96_, opts_1_auto)
  end
  local opts_1_auto
  do
    local tbl_16_auto = {}
  ...
  return vim.keymap.set("n", "<leader>bo", _100_, opts_1_auto)
end
...
return use("folke/lazy.nvim", {event = "VeryLazy"}, "Olical/nfnl", {branch = "main", lazy = false}, "Olical/conjure", {branch = "main", mod = "conjure", lazy = false}, "nvim-lua/plenary.nvim", {}, "nvim-tree/nvim-web-devicons", {mod = "devicons", cond = false}, "rhcher/srcery.nvim", {priority = 1000, lazy = false}, "wtfox/jellybeans.nvim", {priority = 1000, opts = {}, cond = false, lazy = false}, "rhcher/vim-paper", {priority = 1000, lazy = false}, "nvim-lualine/lualine.nvim", {event = "VeryLazy", config = true, dependencies = "nvim-tree/nvim-web-devicons"}, "junegunn/fzf", {event = "VeryLazy", build = "./install --bin"}, "karb94/neoscroll.nvim", {event = "VeryLazy", config = true, cond = false}, "gbprod/substitute.nvim", {config = true, keys = {{"<leader>x", "<cmd>lua require('substitute.exchange').operator()<cr>"}, {"<leader>xx", "<cmd>lua require('substitute.exchange').line()<cr>"}, {"X", "<cmd>lua require('substitute.exchange').visual()<cr>", mode = "x"}, {"<leader>xc", "<cmd>lua require('substitute.exchange').cancel()<cr>"}}}, "nvim-treesitter/nvim-treesitter", {build = ":TSUpdate", mod = "treesitter", version = false}, "eraserhd/parinfer-rust", {ft = util["lisp-language"], config = _7_, build = "cargo build --release"}, "harrygallagher4/nvim-parinfer-rust", {ft = util["lisp-language"], config = _8_}, "saghen/blink.cmp", {opts = {keymap = {preset = "super-tab", ["<C-y>"] = {"select_and_accept", "fallback"}, ["<Tab>"] = {"select_and_accept", "snippet_forward", "fallback"}}, appearance = {use_nvim_cmp_as_default = true, nerd_font_variant = "mono"}, completion = {trigger = {prefetch_on_insert = false}, list = {selection = {preselect = false}}}, sources = {default = {"lsp", "path", "buffer"}}, signature = {enabled = true}}, build = "cargo build --release", version = false}, "onsails/lspkind-nvim", {event = "LspAttach"}, "dcampos/nvim-snippy", {config = "cond", [false] = _10_}, "neovim/nvim-lspconfig", {dependencies = {"saghen/blink.cmp"}, mod = "nvim-lspconfig", lazy = false}, "mfussenegger/nvim-dap", {mod = "dap", dependencies = {"rcarriga/nvim-dap-ui", "theHamsta/nvim-dap-virtual-text"}, lazy = false}, "rcarriga/nvim-dap-ui", {keys = {{"<leader>du", _21_}, {"<leader>de", _22_, mode = {"n", "v"}}}, dependencies = {"mfussenegger/nvim-dap", "nvim-neotest/nvim-nio"}, mod = "dap"}, "theHamsta/nvim-dap-virtual-text", {opts = {}, lazy = false}, "nvimdev/lspsaga.nvim", {dependencies = {"nvim-tree/nvim-web-devicons", "nvim-treesitter/nvim-treesitter"}, event = "LspAttach", mod = "lspsaga"}, "Wansmer/symbol-usage.nvim", {event = "LspAttach", opts = {vt_position = "end_of_line"}, cond = false}, "AndrewRadev/sideways.vim", {event = "VeryLazy", init = _23_}, "altermo/ultimate-autopair.nvim", {event = {"ModeChanged *:[vi]"}, branch = "v0.6", dependencies = {"nvim-treesitter/nvim-treesitter"}, mod = "auto-pairs"}, "stevearc/oil.nvim", {event = "VeryLazy", dependencies = {"nvim-tree/nvim-web-devicons"}, opts = {columns = {"icon", "permissions", "size", "mtime"}}, cond = false}, "utilyre/sentiment.nvim", {event = "VeryLazy", opts = {}, version = false}, "ibhagwan/fzf-lua", {cmd = "FzfLua", dependencies = {"nvim-tree/nvim-web-devicons"}, opts = {winopts = {split = "bot new", border = "single"}, copen = "bot new"}, init = _30_, cond = false}, "tpope/vim-fugitive", {cmd = "Git"}, "junegunn/gv.vim", {cmd = "GV", dependencies = {"tpope/vim-fugitive"}}, "julienvincent/nvim-paredit", {ft = util["lisp-language"], mod = "nvim-paredit"}, "svban/YankAssassin.vim", {event = "VeryLazy"}, "kana/vim-textobj-user", {event = "VeryLazy", dependencies = {"glts/vim-textobj-comment"}}, "chrisgrieser/nvim-various-textobjs", {keys = {{"iv", "<cmd>lua require('various-textobjs').subword('inner')<CR>", mode = {"o", "x"}}, {"av", "<cmd>lua require('various-textobjs').subword('outer')<CR>", mode = {"o", "x"}}}}, "AndrewRadev/linediff.vim", {cmd = "Linediff"}, "tyru/open-browser.vim", {keys = {{"gx", "<Plug>(openbrowser-smart-search)", mode = {"n", "v"}}}}, "kevinhwang91/nvim-fundo", {dependencies = {"kevinhwang91/promise-async"}, event = "VeryLazy", build = _71_, config = true}, "kevinhwang91/nvim-ufo", {dependencies = {"kevinhwang91/promise-async", {"luukvbaal/statuscol.nvim", config = _72_}}, event = "VeryLazy", config = _73_}, "kevinhwang91/nvim-bqf", {ft = "qf", init = _81_, mod = "bqf"}, "RRethy/vim-illuminate", {event = {"BufReadPost", "BufNewFile"}, config = _82_}, "lukas-reineke/indent-blankline.nvim", {event = "VeryLazy", mod = "indent", cond = false}, "mbbill/undotree", {cmd = "UndotreeShow"}, "NvChad/nvim-colorizer.lua", {cmd = "ColorizerToggle", main = "colorizer", config = true}, "tpope/vim-repeat", {event = "VeryLazy"}, "kylechui/nvim-surround", {event = "VeryLazy", config = _83_}, "mhinz/vim-grepper", {keys = {"gs"}, mod = "grepper"}, "lewis6991/gitsigns.nvim", {event = {"BufReadPre", "BufNewFile"}, mod = "gitsigns"}, "akinsho/nvim-toggleterm.lua", {keys = {"<leader>ot", "<leader>oT"}, mod = "terminal"}, "folke/flash.nvim", {event = "VeryLazy", opts = {modes = {char = {enabled = false}}}, keys = {{"s", _86_, mode = {"n", "x", "o"}}, {"S", _87_, mode = {"n", "x", "o"}}, {"r", _88_, mode = "o"}, {"R", _89_, mode = {"x", "o"}}, {"<C-s>", _90_, mode = "c"}}}, "booperlv/nvim-gomove", {event = "VeryLazy", mod = "move"}, "zegervdv/nrpattern.nvim", {keys = {"<C-a>", "<C-x>"}, config = _91_}, "ThePrimeagen/harpoon", {keys = {"<leader>uu", "<leader>ua", "<leader>un", "<leader>up", "<leader>tc"}, dependencies = {"nvim-lua/plenary.nvim"}, mod = "harpoon"}, "danymat/neogen", {cmd = "Neogen", opts = {snippet_engine = "snippy"}}, "kazhala/close-buffers.nvim", {keys = {"<leader>bk", "<leader>bo"}, config = _92_}, "anuvyklack/hydra.nvim", {event = "VeryLazy", dependencies = {"anuvyklack/keymap-layer.nvim", "mrjones2014/smart-splits.nvim"}, mod = "hydra"}, "cshuaimin/ssr.nvim", {keys = {"<leader>sr"}, config = _101_}, "dhruvasagar/vim-table-mode", {cmd = "TableModeToggle", init = _105_}, "nvimdev/dashboard-nvim", {event = "VimEnter", mod = "dashboard", dependencies = {"nvim-tree/nvim-web-devicons"}, cond = false}, "gbprod/cutlass.nvim", {event = "VeryLazy", opts = {}}, "kevinhwang91/nvim-hlslens", {event = "VeryLazy", mod = "hlslens", dependencies = {"haya14busa/vim-asterisk"}}, "mg979/vim-visual-multi", {event = "VeryLazy"}, "max397574/better-escape.nvim", {event = "VeryLazy", config = _106_}, "sindrets/diffview.nvim", {cmd = "DiffviewOpen"}, "chrisgrieser/nvim-spider", {keys = {{"w", "<cmd>lua require('spider').motion('w')<CR>", mode = {"n", "o", "x"}}, {"e", "<cmd>lua require('spider').motion('e')<CR>", mode = {"n", "o", "x"}}, {"b", "<cmd>lua require('spider').motion('b')<CR>", mode = {"n", "o", "x"}}, {"ge", "<cmd>lua require('spider').motion('ge')<CR>", mode = {"n", "o", "x"}}}}, "tiagovla/scope.nvim", {config = true, lazy = false}, "nvim-dd.git", {url = "https://gitlab.com/yorickpeterse/nvim-dd.git", event = "VeryLazy", opts = {timeout = 200}}, "willothy/flatten.nvim", {config = true, priority = 1001, lazy = false}, "Julian/lean.nvim", {event = {"BufReadPre *.lean", "BufNewFile *.lean"}, dependencies = {"neovim/nvim-lspconfig", "nvim-lua/plenary.nvim"}, opts = {lsp = {}, mappings = true}}, "OXY2DEV/markview.nvim", {dependencies = {"nvim-treesitter/nvim-treesitter", "nvim-tree/nvim-web-devicons"}, lazy = false}, "folke/snacks.nvim", {priority = 1000, mod = "snacks", lazy = false}, "echasnovski/mini.nvim", {keys = {{"<leader>e", "<cmd>lua MiniFiles.open()<CR>"}}, mod = "mini", version = false})
