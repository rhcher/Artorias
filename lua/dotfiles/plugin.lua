...
local function _77_()
  ...
  local function _84_(bufnr, filetype, buftype)
    return ftmap[filetype]
  end
  return ufo.setup({provider_selector = _84_})
end
local function _85_()
...
local function _87_()
  ...
  return surround.setup({surrounds = {["("] = {add = {"(", ")"}, find = _88_, delete = "^(.)().-(.)()$"}, [")"] = {add = {"( ", " )"}, find = _89_, delete = "^(. ?)().-( ?.)()$"}}, move_cursor = false})
end
local function _90_()
...
local function _96_()
  ...
  do
    ...
    vim.keymap.set("n", "<leader>bk", _100_, opts_1_auto)
  end
  local opts_1_auto
  do
    local tbl_14_auto = {}
  ...
  return vim.keymap.set("n", "<leader>bo", _104_, opts_1_auto)
end
...
return use("folke/lazy.nvim", {event = "VeryLazy"}, "Olical/nfnl", {ft = "fennel"}, "Olical/conjure", {branch = "develop", mod = "conjure", lazy = false}, "nvim-lua/plenary.nvim", {}, "nvim-tree/nvim-web-devicons", {mod = "devicons"}, "rhcher/srcery.nvim", {priority = 1000, config = _7_, lazy = false}, "rhcher/vim-paper", {priority = 1000}, "rebelot/kanagawa.nvim", {priority = 1000}, "nvimdev/whiskyline.nvim", {event = "VimEnter", opts = {bg = "#2a2a47"}, dependencies = {"nvim-tree/nvim-web-devicons"}}, "junegunn/fzf", {event = "VeryLazy", build = "./install --bin"}, "karb94/neoscroll.nvim", {event = "VeryLazy", config = true}, "gbprod/substitute.nvim", {config = true, keys = {{"<leader>x", "<cmd>lua require('substitute.exchange').operator()<cr>"}, {"<leader>xx", "<cmd>lua require('substitute.exchange').line()<cr>"}, {"X", "<cmd>lua require('substitute.exchange').visual()<cr>", mode = "x"}, {"<leader>xc", "<cmd>lua require('substitute.exchange').cancel()<cr>"}}}, "rcarriga/nvim-notify", {keys = {{"<leader>un", _8_}}, opts = {timeout = 3000, background_colour = "#000000", max_height = _9_, max_width = _10_}, cond = false}, "folke/noice.nvim", {event = "VeryLazy", dependencies = {"MunifTanjim/nui.nvim"}, keys = {{"<S-Enter>", _11_, mode = "c"}}, opts = {lsp = {progress = {enabled = false}, hover = {enabled = false}, override = {["vim.lsp.util.convert_input_to_markdown_lines"] = false, ["cmp.entry.get_documentation"] = false, ["vim.lsp.util.stylize_markdown"] = false}}, presets = {bottom_search = true, command_palette = true, long_message_to_split = true}, messages = {enabled = false}}}, "nvim-treesitter/nvim-treesitter", {build = ":TSUpdate", mod = "treesitter", version = false}, "eraserhd/parinfer-rust", {ft = util["lisp-language"], config = _12_, build = "cargo build --release"}, "harrygallagher4/nvim-parinfer-rust", {ft = util["lisp-language"], config = _13_}, "hrsh7th/nvim-cmp", {event = {"InsertEnter"}, dependencies = {"hrsh7th/cmp-nvim-lsp", "hrsh7th/cmp-buffer", "hrsh7th/cmp-path", "PaterJason/cmp-conjure", "hrsh7th/cmp-cmdline", "onsails/lspkind-nvim"}, mod = "cmp", version = false}, "onsails/lspkind-nvim", {event = "LspAttach"}, "hrsh7th/cmp-cmdline", {event = "CmdlineEnter"}, "dcampos/nvim-snippy", {event = "VeryLazy", init = _15_}, "neovim/nvim-lspconfig", {dependencies = {"hrsh7th/cmp-nvim-lsp"}, mod = "nvim-lspconfig", lazy = false}, "mfussenegger/nvim-dap", {mod = "dap", dependencies = {"rcarriga/nvim-dap-ui", "theHamsta/nvim-dap-virtual-text"}, lazy = false}, "rcarriga/nvim-dap-ui", {keys = {{"<leader>du", _26_}, {"<leader>de", _27_, mode = {"n", "v"}}}, mod = "dap"}, "theHamsta/nvim-dap-virtual-text", {opts = {}, lazy = false}, "nvimdev/lspsaga.nvim", {dependencies = {"nvim-tree/nvim-web-devicons", "nvim-treesitter/nvim-treesitter"}, event = "LspAttach", mod = "lspsaga"}, "Wansmer/symbol-usage.nvim", {event = "LspAttach", opts = {vt_position = "end_of_line"}, cond = false}, "AndrewRadev/sideways.vim", {event = "VeryLazy", init = _28_}, "altermo/ultimate-autopair.nvim", {event = {"InsertEnter", "CmdlineEnter"}, branch = "v0.6", dependencies = {"nvim-treesitter/nvim-treesitter"}, mod = "auto-pairs"}, "stevearc/oil.nvim", {event = "VeryLazy", dependencies = {"nvim-tree/nvim-web-devicons"}, opts = {columns = {"icon", "permissions", "size", "mtime"}}}, "utilyre/sentiment.nvim", {version = "*", event = "VeryLazy", opts = {}}, "ibhagwan/fzf-lua", {cmd = "FzfLua", dependencies = {"nvim-tree/nvim-web-devicons"}, opts = {winopts = {split = "bot new", border = "single"}, copen = "bot new"}, init = _35_}, "tpope/vim-fugitive", {cmd = "Git"}, "junegunn/gv.vim", {cmd = "GV", dependencies = {"tpope/vim-fugitive"}}, "julienvincent/nvim-paredit", {ft = util["lisp-language"], mod = "nvim-paredit"}, "julienvincent/nvim-paredit-fennel", {ft = {"fennel"}, dependencies = "julienvincent/nvim-paredit"}, "svban/YankAssassin.vim", {event = "VeryLazy"}, "kana/vim-textobj-user", {event = "VeryLazy", dependencies = {"glts/vim-textobj-comment"}}, "chrisgrieser/nvim-various-textobjs", {keys = {{"iv", "<cmd>lua require('various-textobjs').subword('inner')<CR>", mode = {"o", "x"}}, {"av", "<cmd>lua require('various-textobjs').subword('outer')<CR>", mode = {"o", "x"}}}}, "AndrewRadev/linediff.vim", {cmd = "Linediff"}, "tyru/open-browser.vim", {keys = {{"gx", "<Plug>(openbrowser-smart-search)", mode = {"n", "v"}}}}, "kevinhwang91/nvim-ufo", {dependencies = {"kevinhwang91/promise-async", {"luukvbaal/statuscol.nvim", config = _76_}}, event = "VeryLazy", config = _77_}, "kevinhwang91/nvim-bqf", {ft = "qf", init = _85_, mod = "bqf"}, "RRethy/vim-illuminate", {event = {"BufReadPost", "BufNewFile"}, config = _86_}, "lukas-reineke/indent-blankline.nvim", {event = "VeryLazy", branch = "v3", dependencies = {"nvim-treesitter/nvim-treesitter"}, mod = "indent", cond = false}, "numToStr/Comment.nvim", {event = "VeryLazy", config = true}, "mbbill/undotree", {cmd = "UndotreeShow"}, "NvChad/nvim-colorizer.lua", {cmd = "ColorizerToggle", main = "colorizer", config = true}, "tpope/vim-repeat", {event = "VeryLazy"}, "kylechui/nvim-surround", {event = "VeryLazy", config = _87_}, "mhinz/vim-grepper", {keys = {"gs"}, mod = "grepper"}, "lewis6991/gitsigns.nvim", {event = {"BufReadPre", "BufNewFile"}, mod = "gitsigns"}, "akinsho/nvim-toggleterm.lua", {keys = {"<leader>ot", "<leader>oT"}, mod = "terminal"}, "folke/flash.nvim", {event = "VeryLazy", opts = {}, keys = {{"s", _90_, mode = {"n", "x", "o"}}, {"S", _91_, mode = {"n", "x", "o"}}, {"r", _92_, mode = "o"}, {"R", _93_, mode = {"x", "o"}}, {"<C-s>", _94_, mode = "c"}}}, "booperlv/nvim-gomove", {event = "VeryLazy", mod = "move"}, "zegervdv/nrpattern.nvim", {keys = {"<C-a>", "<C-x>"}, config = _95_}, "ThePrimeagen/harpoon", {keys = {"<leader>uu", "<leader>ua", "<leader>un", "<leader>up", "<leader>tc"}, dependencies = {"nvim-lua/plenary.nvim"}, mod = "harpoon"}, "danymat/neogen", {cmd = "Neogen", opts = {snippet_engine = "snippy"}}, "kazhala/close-buffers.nvim", {keys = {"<leader>bk", "<leader>bo"}, config = _96_}, "anuvyklack/hydra.nvim", {event = "VeryLazy", dependencies = {"anuvyklack/keymap-layer.nvim", "mrjones2014/smart-splits.nvim"}, mod = "hydra"}, "cshuaimin/ssr.nvim", {keys = {"<leader>sr"}, config = _105_}, "dhruvasagar/vim-table-mode", {cmd = "TableModeToggle", init = _109_}, "nvimdev/dashboard-nvim", {event = "VimEnter", mod = "dashboard", dependencies = {"nvim-tree/nvim-web-devicons"}}, "gbprod/cutlass.nvim", {event = "VeryLazy", opts = {}}, "kevinhwang91/nvim-hlslens", {event = "VeryLazy", mod = "hlslens", dependencies = {"haya14busa/vim-asterisk"}}, "mg979/vim-visual-multi", {event = "VeryLazy"}, "max397574/better-escape.nvim", {event = "VeryLazy", config = _110_}, "sindrets/diffview.nvim", {cmd = "DiffviewOpen"}, "chrisgrieser/nvim-spider", {keys = {{"w", "<cmd>lua require('spider').motion('w')<CR>", mode = {"n", "o", "x"}}, {"e", "<cmd>lua require('spider').motion('e')<CR>", mode = {"n", "o", "x"}}, {"b", "<cmd>lua require('spider').motion('b')<CR>", mode = {"n", "o", "x"}}, {"ge", "<cmd>lua require('spider').motion('ge')<CR>", mode = {"n", "o", "x"}}}}, "tiagovla/scope.nvim", {config = true, lazy = false})
