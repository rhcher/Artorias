-- [nfnl] Compiled from fnl/dotfiles/plugin/cmp.fnl by https://github.com/Olical/nfnl, do not edit.
...
if ok_3f then
  ...
  cmp.setup({snippet = {expand = _5_}, completion = {completeopt = "menu,menuone,noselect", keyword_pattern = "\\k\\+"}, sorting = {comparators = {cmp.config.compare.offset, cmp.config.compare.exact, cmp.config.compare.score, cmp.config.compare.recently_used, cmp.config.compare.locality, cmp.config.compare.length, cmp.config.compare.order}}, experimental = {ghost_text = {hl_group = "LspCodeLens"}}, window = {documentation = cmp_window_opts, completion = cmp_window_opts}, mapping = cmp.mapping.preset.insert({["<C-d>"] = cmp.mapping.scroll_docs(( - 4)), ["<C-f>"] = cmp.mapping.scroll_docs(4), ["<C-space>"] = cmp.mapping.complete(), ["<C-e>"] = cmp.mapping.close(), ["<C-y>"] = cmp.mapping.confirm({select = true, behavior = cmp.ConfirmBehavior.Replace}), ["<Tab>"] = cmp.mapping(_6_, {"s", "i", "c"})}), formatting = {fields = {"kind", "abbr", "menu"}, format = lspkind.cmp_format({mode = "symbol", maxwidth = 50, before = _8_})}, sources = cmp.config.sources({{name = "nvim_lsp"}}, {{name = "path"}, cmp_buffer})})
